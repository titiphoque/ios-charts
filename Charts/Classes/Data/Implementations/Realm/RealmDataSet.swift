...
public class RealmDataSet: NSObject, IChartDataSet
{
    ...
    public init(results: RLMResults?, yValuesField: String, xIndexField: String, label: String?)
    {
        super.init()
    ...
    public convenience init(results: RLMResults, yValuesField: String, xIndexField: String)
    {
        self.init(results: results, yValuesField: yValuesField, xIndexField: xIndexField, label: "DataSet")
    }
    
    /*public init(realm: RLMRealm?, modelName: String, resultsWhere: String, yValuesField: String, xIndexField: String, label: String?)
    {
        super.init()
    ...
    {
        loadResults(realm: realm, modelName: modelName, resultsWhere: nil)
    }
    
    public func loadResults(realm realm: RLMRealm, modelName: String, resultsWhere: String?)
    {
        if resultsWhere == nil
        {
        ...
        {
            _results = realm.objects(modelName, withPredicate: NSPredicate(format: resultsWhere!, arguments: nil))
        }
    ...
    internal var _yValuesField: String?
    ...
    internal func ensureCache(start start: Int, end: Int)
    {
        ...
        guard let yValuesField = _yValuesField,
            results = _results,
        ...
        if _cacheFirst == -1 || _cacheLast == -1
        {
            ...
            for (var i = UInt(start), end = UInt(end + 1); i < end; i++)
            {
                let object = results.objectAtIndex(i)
                
                let entry = ChartDataEntry(value: object[yValuesField] as! Double, xIndex: object[xIndexField] as! Int)
                _cache.append(entry)
            }
        ...
        if start < _cacheFirst
        {
            ...
            for (var i = UInt(start), end = UInt(_cacheFirst); i < end; i++)
            {
                let object = results.objectAtIndex(i)
        ...
        if end > _cacheLast
        {
            for (var i = UInt(_cacheLast), end = UInt(end + 1); i < end; i++)
            {
                let object = results.objectAtIndex(i)
    ...
    {
        ensureCache(start: start, end: end)
        
        let yValCount = _cache.count
    ...
    {
        _results.filter(need to create a filter string here to find index by xIndex)
    }
    ...
    public func copyWithZone(zone: NSZone) -> AnyObject
    {
        ...
        copy._yValuesField = _yValuesField
